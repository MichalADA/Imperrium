...
const catalogCopy: Record<EntryType, { title: string; eyebrow: string; description: string }> = {
  ...
  GROUP:{title:"Grupy Wielkich Rodów",eyebrow:"Kręgi towarzyskie",description:"Paczki, kręgi i nieformalne grupy związane z Wielkimi Rodami oraz najwyższymi elitami Imperium."},
...
function DynamicCatalog(){const{type="CHARACTER"}=useParams();const normalized=type.toUpperCase() as EntryType;if(!catalogCopy[normalized])return <NotFoundPage/>;return <CatalogPage type={normalized}{...catalogCopy[normalized]}/>;}
export default function App(){return <Routes><Route element={<Layout/>}><Route index element={<DashboardPage/>}/><Route path="postacie" element={<CatalogPage type="CHARACTER" {...catalogCopy.CHARACTER}/>}/><Route path="postacie/:slug" element={<EntryDetailPage/>}/><Route path="prowincje" element={<CatalogPage type="PROVINCE" {...catalogCopy.PROVINCE}/>}/><Route path="prowincje/:slug" element={<EntryDetailPage/>}/><Route path="jezyki" element={<CatalogPage type="LANGUAGE" {...catalogCopy.LANGUAGE}/>}/><Route path="firmy" element={<CatalogPage type="COMPANY" {...catalogCopy.COMPANY}/>}/><Route path="grupy" element={<CatalogPage type="GROUP" {...catalogCopy.GROUP}/>}/><Route path="dynastia" element={<DynastyPage/>}/><Route path="chronologia" element={<TimelinePage/>}/><Route path="szukaj" element={<SearchPage/>}/><Route path="katalog/:type" element={<DynamicCatalog/>}/><Route path="wpis/:slug" element={<EntryDetailPage/>}/><Route path="admin/nowy" element={<EditorPage/>}/><Route path="admin/edytuj/:slug" element={<EditorPage/>}/><Route path="historia" element={<Navigate to="/katalog/EVENT" replace/>}/><Route path="*" element={<NotFoundPage/>}/></Route></Routes>;}
